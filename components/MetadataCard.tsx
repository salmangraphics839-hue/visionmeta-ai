...
import { CheckCircle, Loader2, AlertCircle, Edit2, Save, X, Copy, Check, ArrowUp, Bookmark, Copy as CopyIcon, Sparkles, Plus, Layers, Eye, Briefcase, Zap, BrainCircuit, FileText, Wand2, RefreshCw } from 'lucide-react';
...
const MetadataCard: React.FC<MetadataCardProps> = ({ 
  ...
  // SAFE GUARD: Ensure keywords is always an array
  const safeKeywords = item.metadata?.keywords || [];
  const safeTitle = item.metadata?.title || "";
  const safeDesc = item.metadata?.description || "";

  useEffect(() => {
    if (item.metadata) {
      setEditTitle(safeTitle);
      setEditDesc(safeDesc);
  ...
  const promoteKeyword = (index: number) => {
    if (index === 0) return;
    const currentKeywords = isEditing ? editKeywords : safeKeywords;
  ...
  const handleStrategicAnalysis = async () => {
      ...
      try {
        ...
        const report = await generateStrategicAnalysis("", base64, item.file.type, safeTitle || "Image");
  ...
  const isVideo = item.file.type.startsWith('video/') || item.file.name.match(/\.(mp4|mov|webm|avi)$/i);
  ...
  return (
    <div className={`bg-slate-800 border rounded-xl overflow-hidden flex flex-col md:flex-row shadow-lg transition-all hover:shadow-2xl group/card ${isSelected ? 'border-cyan-500 ring-1 ring-cyan-500/50' : 'border-slate-700 hover:border-slate-600'}`}>
      <div className="w-full md:w-64 h-64 md:h-auto relative bg-slate-900 flex-shrink-0">
        {isVideo ? (
            <video src={item.previewUrl} controls className="w-full h-full object-contain bg-slate-950/50" />
        ) : (
            <img src={item.previewUrl} alt="Preview" className="w-full h-full object-contain bg-slate-950/50" />
        )}
        ...
        {item.status === FileStatus.SUCCESS && item.metadata && !isEditing && (
             <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-slate-900 to-transparent p-3 opacity-0 group-hover/card:opacity-100 transition-opacity flex justify-center gap-2">
                <button onClick={() => onApplyToAll(item.metadata!)} className="bg-slate-800/90 hover:bg-cyan-600 text-slate-300 hover:text-white p-2 rounded-full backdrop-blur border border-slate-600 hover:border-cyan-500 transition-all shadow-lg">
                  <CopyIcon className="w-4 h-4" />
                </button>
                <button onClick={() => onSavePreset(item.metadata!)} className="bg-slate-800/90 hover:bg-purple-600 text-slate-300 hover:text-white p-2 rounded-full backdrop-blur border border-slate-600 hover:border-purple-500 transition-all shadow-lg">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-slate-800/50">
        ...
        {item.status === FileStatus.SUCCESS && (
            <div className="flex border-b border-slate-700 bg-slate-900/30">
                <button onClick={() => setActiveTab('metadata')} className={`flex-1 py-2 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 border-b-2 transition-colors ${activeTab === 'metadata' ? 'border-cyan-500 text-cyan-400' : 'border-transparent text-slate-500 hover:text-slate-300'}`}>
                    <FileText className="w-3 h-3" /> Metadata
                </button>
                <button onClick={() => { setActiveTab('strategy'); handleStrategicAnalysis(); }} className={`flex-1 py-2 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 border-b-2 transition-colors ${activeTab === 'strategy' ? 'border-amber-500 text-amber-400' : 'border-transparent text-slate-500 hover:text-slate-300'}`}>
                    <BrainCircuit className="w-3 h-3" /> Analysis
                </button>
                <button onClick={() => { setActiveTab('prompt'); }} className={`flex-1 py-2 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 border-b-2 transition-colors ${activeTab === 'prompt' ? 'border-pink-500 text-pink-400' : 'border-transparent text-slate-500 hover:text-slate-300'}`}>
        ...
        {item.status === FileStatus.IDLE && (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-500 space-y-4 h-full">
                ...
                <button onClick={onRegenerate} className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-bold text-sm shadow-lg shadow-blue-900/20 transition-all flex items-center gap-2">
                    <RefreshCw className="w-4 h-4" /> Generate Metadata
                </button>
            </div>
        )}
            ...
            <>
                    ...
                    <div className="flex justify-between items-start">
                        <div className="space-y-1 flex-1 mr-4">
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => copyToClipboard(safeTitle, 'title')} className="text-slate-500 hover:text-cyan-400">
                                    {copiedField === 'title' ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                                </button>
                            </div>
                            <p className="text-slate-100 font-medium leading-snug">{safeTitle}</p>
                        </div>
                        <div className="flex items-center gap-1">
                            <button onClick={onRegenerate} className="text-slate-400 hover:text-cyan-400 p-1.5 rounded-lg hover:bg-slate-700 transition-colors" title="Regenerate Metadata">
                ...
                <div className="space-y-1">
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => copyToClipboard(safeDesc, 'desc')} className="text-slate-500 hover:text-cyan-400">
                             {copiedField === 'desc' ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                        </button>
                    </div>
                    <p className="text-slate-300 text-sm leading-relaxed line-clamp-2">{safeDesc}</p>
                </div>
                ...
                <div className="flex-1 flex flex-col min-h-0">
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                            <h3 className="text-[10px] font-bold text-cyan-400 uppercase tracking-wider">
                                Keywords <span className="text-slate-500 ml-1">({safeKeywords.length})</span>
                            </h3>
                            <button onClick={() => copyToClipboard(safeKeywords.join(', '), 'kw')} className="text-slate-500 hover:text-cyan-400">
                    ...
                    <div className="flex flex-wrap gap-1.5 content-start">
                        {safeKeywords.slice(0, 10).map((kw, i) => (
                        ...
                        {safeKeywords.slice(10).map((kw, i) => (
                            <button key={`rest-${i}`} onClick={() => promoteKeyword(i + 10)} className="group/tag bg-slate-700/50 hover:bg-slate-700 text-slate-300 hover:text-white text-xs px-2 py-1 rounded border border-slate-600/50 hover:border-cyan-500/50 transition-colors cursor-pointer flex items-center gap-1">
                                {kw}
                    ...
                    <div className="mt-3 flex flex-col items-end">
                      {!showQuickSuggest && (
                        <button onClick={() => { setShowQuickSuggest(true); if (suggestedKeywords.length === 0) handleSuggest('mixed'); }} className="text-[10px] font-bold uppercase tracking-wider text-cyan-400 border border-cyan-500/30 bg-cyan-500/10 hover:bg-cyan-500 hover:text-white px-3 py-1.5 rounded-full transition-all flex items-center gap-1.5 shadow-sm hover:shadow-cyan-500/20">
                                ...
                                {isSuggesting ? (
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {suggestedKeywords.map((kw, i) => (
                                            <button key={i} onClick={() => addQuickSuggestion(kw)} className="bg-slate-800 hover:bg-emerald-600 hover:border-emerald-500 text-slate-300 hover:text-white border border-slate-600/50 text-xs px-2.5 py-1.5 rounded-full flex items-center gap-1.5 transition-all group animate-in zoom-in-50 duration-200">
                                  ...
                                  <div className="text-center py-4">
                                     <button onClick={() => handleSuggest(suggestionType)} className="text-xs text-slate-500 hover:text-cyan-400 flex items-center justify-center gap-1 mx-auto">
            ...
            <div className="flex-1 overflow-y-auto pr-2 animate-in fade-in duration-300">
                {isGeneratingPrompt ? (
                    ...
                    <div className="space-y-4">
                        ...
                        <button onClick={() => copyToClipboard(item.generatedPrompt || "", "prompt_copy")} className="w-full py-2 bg-pink-600 hover:bg-pink-500 text-white rounded-lg text-xs font-bold uppercase tracking-wide flex items-center justify-center gap-2 transition-all shadow-lg shadow-pink-900/20">
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-slate-500 gap-4">
                        ...
                        <button onClick={handlePromptGeneration} className="px-4 py-2 bg-pink-600 hover:bg-pink-500 text-white rounded-lg text-xs font-bold uppercase tracking-wide flex items-center gap-2 transition-all shadow-lg shadow-pink-900/20">
        ...
        {isEditing && activeTab === 'metadata' && (
            <div className="flex flex-col gap-4 h-full animate-in fade-in duration-200">
                <div className="space-y-1">
                    ...
                    <input className="w-full bg-slate-900 border border-slate-600 rounded p-2 text-white text-sm focus:border-cyan-500 outline-none" value={editTitle} onChange={(e) => setEditTitle(e.target.value)} />
                </div>
                <div className="space-y-1">
                    <label className="text-xs text-cyan-400 font-bold uppercase tracking-wider">Description</label>
                    <textarea className="w-full bg-slate-900 border border-slate-600 rounded p-2 text-white text-sm focus:border-cyan-500 outline-none resize-none h-20" value={editDesc} onChange={(e) => setEditDesc(e.target.value)} />
                </div>
                <div className="flex-1 flex flex-col min-h-0 space-y-1">
                    ...
                    <div className="w-full bg-slate-900 border border-slate-600 rounded p-2 flex flex-wrap gap-2 content-start overflow-y-auto max-h-[250px]">
                        ...
                        <input className="bg-transparent border-none outline-none text-white text-xs min-w-[100px] flex-1 py-1 placeholder:text-slate-600" placeholder="Type & Enter..." value={newKeywordInput} onChange={(e) => setNewKeywordInput(e.target.value)} onKeyDown={addKeyword} />
                    </div>
                </div>
