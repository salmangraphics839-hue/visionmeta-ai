...
const MetadataCard: React.FC<MetadataCardProps> = ({ 
  ...
  const promoteKeyword = (index: number) => {
    ...
    if (isEditing) {
        ...
        onUpdateMetadata(item.id, { ...safeMetadata, keywords: newKeywords });
    }
  };
  ...
  const handleStrategicAnalysis = async () => {
      ...
      try {
        ...
        const report = await generateStrategicAnalysis("", base64, item.file.type, safeMetadata.title || "Image");
  ...
  const addQuickSuggestion = (kw: string) => {
    const newKeywords = [...safeKeywords, kw];
    onUpdateMetadata(item.id, { ...safeMetadata, keywords: newKeywords });
            ...
            <>
                    ...
                    <div className="flex justify-between items-start">
                        <div className="space-y-1 flex-1 mr-4">
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => copyToClipboard(safeMetadata.title, 'title')} className="text-slate-500 hover:text-cyan-400">
                                    {copiedField === 'title' ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                                </button>
                            </div>
                            <p className="text-slate-100 font-medium leading-snug">{safeMetadata.title || "No Title Generated"}</p>
                        </div>
                ...
                <div className="space-y-1">
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => copyToClipboard(safeMetadata.description, 'desc')} className="text-slate-500 hover:text-cyan-400">
                             {copiedField === 'desc' ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                        </button>
                    </div>
                    <p className="text-slate-300 text-sm leading-relaxed line-clamp-2">{safeMetadata.description || "No Description Generated"}</p>
                </div>
                ...
                <div className="flex-1 flex flex-col min-h-0">
                    ...
                    <div className="flex flex-wrap gap-1.5 content-start">
                        {safeKeywords.length === 0 && <span className="text-slate-500 text-xs italic">No keywords generated. Click 'Suggest More' to add some.</span>}
